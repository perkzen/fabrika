 ...
 * do anything is worse than no key. Seventy-eight columns at its longest, so
 * an eighty-column terminal still shows all of it, which is what `^C` buys
 * over `Ctrl-C`; it stays last as the most drastic.
 */
const keys = (editor: boolean): string =>
  `↑↓ select · space fold · PgUp/PgDn scroll · Esc follow${editor ? " · o open" : ""} · ^C interrupt`;
...
export const frame = (
  ...
  return [
    ...
    // The header's second row, dim: an address, read once and copied, not a
    // thing that competes with the run above it.
